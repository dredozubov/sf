(** * Lists: jWorkin g with Structured Data *)
...
Fixpoint filter_natlist (f: nat -> bool) (l: natlist) :=
  match l with
      | []      => []
      | h :: tl => if f h then h :: filter_natlist f tl else filter_natlist f tl
  end.
...
Eval compute in nonzeros [0;1;0;2;3;0;0].

Example test_nonzeros:            nonzeros [0;1;0;2;3;0;0] = [1;2;3].
Proof. reflexivity. Qed.

Fixpoint oddmembers (l:natlist) : natlist :=
  filter_natlist (fun n => oddb n) l.
...
Fixpoint countoddmembers (l:natlist) : nat :=
  match l with
    | [ ] => 0
    | h :: tl => countoddmembers tl + (if oddb h then 1 else 0)
  end.
...
Fixpoint foldnat (f: nat -> natlist -> natlist) (acc: natlist) (l: natlist) :=
  match l with
    | [] => acc
    | h :: tl => foldnat f (f h acc) tl
  end.

Fixpoint alternate' (l1 l2 res : natlist) : natlist :=
  match l1, l2 with
    | [], l => res ++ l
    | l, [] => res ++ l
    | h1 :: tl1, h2 :: tl2 => alternate' tl1 tl2 (res ++ [h1; h2])
  end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  filter_natlist (fun x => negb (beq_nat v x)) s.
...
(* literal subset *)
Fixpoint subset' (cur:bag) (s:bag) (init:bag) : bool :=
  match cur, s with
    | [], _ => true
    | _, [] => false
    | h1 :: tl1, h2 :: tl2 =>
      if beq_nat h1 h2
      then subset' tl1 tl2 init
      else subset' init tl2 init
  end.

Fixpoint subset (s1:bag) (s2:bag) : bool :=
  match s1, s2 with
    | [], _ => true
    | h :: tl, s2 => if member h s2 then subset tl (remove_one h s2) else false
  end.
...
Lemma count_empty : forall (x : nat), count x [] = 0.
...
Lemma count_add_one : forall (x : nat) (n : natlist), count x (x :: n) = 1 + count x n.
Proof.
  induction n.
...
Lemma beq_count_cons : forall (x y : nat) (n : natlist),
    count x (y :: n) = count x [y] + count x n.
Proof.
  induction n.
    - rewrite count_empty, plus_comm. cbn. reflexivity.
    -
Admitted.

(* it is called this way, because 'app_assoc' is going to be defined later in this course *)
Lemma app_assoc0 : forall (x: nat) (m n: natlist), (x :: m) ++ n = x :: m ++ n.
Proof.
  induction m as [|h t]; reflexivity.
Qed.

Theorem count_add : forall (x : nat) (m n : natlist),
                      count x m + count x n = count x (m ++ n).
Proof.
  induction m as [|h tl].
    - cbn; reflexivity.
    - destruct n.
      + rewrite count_empty, plus_comm, app_nil_r. reflexivity.
      + rewrite beq_count_cons.
        assert (Hr: count x tl + count x (n :: n0) = count x (tl ++ (n :: n0))).
          apply IHtl.
        assert (Hassoc: count x [h] + count x tl + count x (n :: n0) =
          count x [h] + (count x tl + count x (n :: n0))).
          rewrite plus_assoc; reflexivity.
        rewrite Hassoc, Hr, app_assoc0.
        assert (Hrew: count x (h :: tl ++ n :: n0) =
                      count x [h] + count x (tl ++ n :: n0)).
          apply beq_count_cons.
        rewrite Hrew.
        reflexivity.
Qed.
...
Proof.
  intros n l. induction l as [| n' tl'].
    Case "l = nil". reflexivity.
    Case "l = snoc tl' n'". simpl. rewrite IHtl'. reflexivity.
  Qed.
(**
...
Proof.
  induction l.
    - simpl. reflexivity.
    - intros. simpl. destruct l as [|h t].
      + simpl. reflexivity.
...
Proof.
  induction l1 as [|h t].
    ...
    - intros. simpl. rewrite IHt. rewrite snoc_append, snoc_append.
...
Proof.
  ...
  induction l1 as [|h t].
    - reflexivity.
    - destruct l2 as [|h' t'].
      + rewrite app_nil_end, app_nil_end. reflexivity.
...
Fixpoint beq_natlist (l1 l2 : natlist) : bool :=
  match l1, l2 with
  | nil,    nil    => true
  | (x::a), (y::b) => if beq_nat x y then beq_natlist a b else false
  | _,      _      => false
  end.
