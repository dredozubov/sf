...
(* this is need for the code below to work with coq 8.5 *)
...
Fixpoint repeat' {X : Type} (n : X) (count : nat) (l : list X) : list X :=
  match count, l with
    | 0, l => l
    | S i, l => n :: repeat' n i l
  end.
...
Proof.
  induction s; simpl; [ reflexivity | rewrite IHs; simpl; reflexivity ].
Qed.
...
Fixpoint split'
         {X Y : Type} (l : list (X*Y)) (acc: list X * list Y)
  : list X * list Y :=
  match l, acc with
    | nil, a => a
    | (x,y) :: t, (m, n) => split' t (m ++ [x], n ++ [y])
  end.
...
Proof.
  simpl. induction p. simpl. reflexivity.
Qed.
...
Fixpoint partition'
         {X : Type} (test : X -> bool) (l : list X) (acc : list X * list X) :
  list X * list X :=
  match l, acc with
    | nil, acc => acc
    | h :: tl, (x,y) => if test h
                       then partition' test tl (x ++ [h], y)
                       else partition' test tl (x, y ++ [h])
  end.
...
Eval compute in snoc (rev [1;2;3]) 0.

Lemma snoc_rev : forall {X: Type} (h: X) (l: list X),
                     snoc (rev l) h = rev (h :: l).
Proof.
Admitted.
...
Fixpoint flat_map' {X Y:Type} (f:X -> list Y) (l:list X) (acc: list Y): (list Y) :=
  match l with
    | nil => nil
    | h :: tl => f h ++ flat_map' f tl acc
  end.
...
Lemma map_hd : forall (X Y: Type) (f: X -> Y) (h: X) (t : list X),
    map f (h :: t) = f h :: map f t.
Proof.
  intros X Y f h t.
  induction t as [|hd tl]; simpl; reflexivity.
Qed.

(** Write down a theorem [fold_map_correct] in Coq stating that
   [fold_map] is correct, and prove it. *)
Lemma fold_map_correct : forall (X : Type) (f : X -> X) (m : list X),
    fold_map f m = map f m.
Proof.
  unfold fold_map.
  induction m as [|h t].
    - reflexivity.
    - rewrite map_hd. rewrite <- IHt. reflexivity.
Qed.
