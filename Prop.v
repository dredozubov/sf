...
Proof.
  intros n H.
  apply b_sum.
  induction n. apply b_0. apply H. rewrite plus_comm. simpl. apply H.
Qed.
...
Proof.
  ...
  intros H.
    - apply b_0.
    - generalize dependent n. intros H.
      intros F G. apply b_sum. apply G.
      apply F, G.
Qed.
...
g_0:     _________________________
              gorgeous 0

                gorgeous n
g_plus3: ________________________
              gorgeous (3 + n)

               gorgeous 5
g_plus5: ________________________
              gorgeous (5 + n)
*)
...
Proof.
  intros n Gm Gn.
  induction Gn as [|n'|n']; intros.
    - simpl. apply H.
    - apply IHGn in H. apply g_plus3, H.
    - apply IHGn in H. apply g_plus5, H.
Qed.
...
Proof.
  ...
  induction G.
    ...
    - apply IHG. inversion H. apply H1.
Qed.
...
(* offtopic: irc discussion *)
Lemma nat_inj { n m : nat } : n = m -> S n = S m. Proof with (congruence).

Inductive pal {X : Type} : list X -> Prop :=
  | pal_nil : pal []
  | pal_sing : forall a : X, pal [a]
  | pal_cons : forall (a : X) (l p : list X), pal p -> pal (l ++ p ++ rev l).
...
Proof.
  ...
  induction l.
    - simpl. apply pal_nil.
    - assert (H: [] ++ rev (x :: l) = rev (x :: l)) by reflexivity.
      rewrite <- H. apply pal_cons; try assumption; apply pal_nil.
Qed.
...
Inductive total_relation : nat -> nat -> Prop :=
  totalr : forall (m n : nat),  total_relation m n.

Example total_relation_eq : total_relation 3 3.
Proof. apply totalr. Qed.
...
Proof.
  intros.
  induction n. reflexivity. apply le_S. assumption.
Qed.
