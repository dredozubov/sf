...
Lemma andb_assoc : forall b c : bool, andb b c = andb c b.
Proof.
  destruct b as [|b'];destruct c as [|c'];reflexivity.
Qed.

   (* (b && false)%bool = false -> b = true *)

Lemma andb_dep_c : forall b c : bool, andb true b = c -> b = c.
Proof.
  intros b.
  intros c.
  simpl.
...
Lemma andb_rev_dep : forall b c : bool, andb true b = c -> b = c.
Proof.
  intros b.
  apply andb_dep_c.
Qed.
...
Proof.
  intros b c.
  induction b as [| b']; intros; apply andb_rev_dep.
  apply H. inversion H.
Qed.
(** [] *)
...
inductive adds an induction hypothesis and will work with all inductive dataypes
...
Lemma nat_inc_S_irr : forall a b, S a = S b -> a = b.
Proof.
  intros a b. intros. inversion H. rewrite <- H1. reflexivity.
Qed.

Lemma mult_S_r : forall m n, n * S m = n + n * m.
Proof.
...
Proof.
  ...
  induction m as [m'|m'].
    ...
    Case "m = m'". rewrite mult_S_r. rewrite <- IHm'. simpl. reflexivity.
Qed.
(** **** Exercise: 2 stars, optional (evenb_n__oddb_Sn)  *)

(** Prove the following simple fact: *)

Lemma eq_double_negb : forall x, negb (negb x) = x.
Proof. intros. destruct x as [ |x']; reflexivity. Qed.
...
Proof.
  ...
  Case "n = n'".
    assert (H : evenb (S (S n')) = evenb n').
      induction n' as [ |n''].
        reflexivity. simpl. destruct n'' as [ |n'''].
          reflexivity. reflexivity.
    assert (H0 : evenb (S (S n')) = evenb n').
      rewrite <- H. reflexivity.
    rewrite H0. induction n' as [ |n'']. reflexivity.
    rewrite IHn'. rewrite eq_double_negb. reflexivity.
Qed.
(** [] *)
